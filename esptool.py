...
class ESPROM:
    ...
    def flash_finish(self, reboot = False):
        ...
        if self.command(ESPROM.ESP_FLASH_END, pkt)[1] != "\0\0":
            print("Failed to leave Flash mode")
#            raise Exception('Failed to leave Flash mode')
        self.dump_serial("serial-dump.out")
        ...
        for l in proc.stdout:
            ...
            if len(fields) >= 3:
                try:
                    ...
                    print "Value " + fields[0] + " is not an integer; field " + fields[2]
            else:
                if len(fields) >= 1:
                    print "Line with field " + fields[len(fields) - 1] + " has wrong format"
                else:
